...
registration_numbers = sorted(df["Registration Number"].unique())
...
# Prepare details for Expired and For Renewal
expired_details = []
renewal_details = []
...
for idx, row in filtered_df.iterrows():
    for doc, status_col in zip(exp_date_column, status_columns):
        if row[status_col] == "Expired":
            expired_details.append({"Equipment Type": row["Equipment Type"], 
                                    "Registration Number": row["Registration Number"], 
                                    "Document Type": doc})
        elif row[status_col] == "For Renewal":
            renewal_details.append({"Equipment Type": row["Equipment Type"], 
                                    "Registration Number": row["Registration Number"], 
                                    "Document Type": doc})
